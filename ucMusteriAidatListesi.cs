...
namespace GPTS
{
    public partial class ucMusteriAidatListesi : DevExpress.XtraEditors.XtraUserControl
    {
        public ucMusteriAidatListesi()
        {
            InitializeComponent();
            DB.FirmaAdi = "";
        }
        private void ucCariKartlar_Load(object sender, EventArgs e)
        {
            ...
            string Dosya = DB.exeDizini + "\\MusteriAidatListesiGrid.xml";
...
//        }
        ...
        void OdemeAl()
        {
            ...
            KasaGirisi.tEaciklama.Text = "Aidat Ödemesi";
        ...
        void OdemeYap()
        {
            ...
            KasaCikis.tEaciklama.Text = "Aidat Borçlandırma";
        ...
        {
            string sql = "";
            //                @"SELECT sec as Sec, Firmalar.pkFirma, Firmalar.Firmaadi,Firmalar.Yetkili, Firmalar.LimitBorc, Firmalar.KaraListe, 
            ...
            //Firmalar.SonOdemeTarihi,Firmalar.KayitTarihi,datediff(dd,SonOdemeTarihi,GETDATE()) as OdemeYapmadigiGun
            ...
            sql = @"select F.pkFirma,F.Firmaadi,F.Yetkili, F.KaraListe,F.Tel,F.Cep,F.AnlasmaTarihi,F.AnlasaBitisTarihi,
FG.GrupAdi,DATEDIFF(day, getdate(), isnull(AnlasaBitisTarihi,getdate())) as kalangun,
sum(kh.Borc) as Borc,sum(kh.Alacak) as Alacak,f.Devir,F.Eposta
 from Firmalar F with(nolock)
left join KasaHareket kh with(nolock) on kh.fkFirma = F.pkFirma
LEFT JOIN FirmaGruplari FG with(nolock) ON FG.pkFirmaGruplari = F.fkFirmaGruplari
--where pkFirma = 805
group by F.pkFirma,F.Firmaadi,F.Yetkili, F.KaraListe,Tel,Cep,AnlasmaTarihi,AnlasaBitisTarihi,FG.GrupAdi,F.Devir,F.Eposta";
        ...
        void barkoddanara()
        {
            string sql = @"select F.pkFirma,F.Firmaadi,F.Yetkili, F.KaraListe,Tel,Cep,AnlasmaTarihi,AnlasaBitisTarihi,
FG.GrupAdi,DATEDIFF(day, getdate(), isnull(AnlasaBitisTarihi,getdate())) as kalangun,
sum(kh.Borc) as Borc,sum(kh.Alacak) as Alacak,f.Devir,F.Eposta
 from Firmalar F with(nolock)
left join KasaHareket kh with(nolock) on kh.fkFirma = F.pkFirma
...
group by F.pkFirma,F.Firmaadi,F.Yetkili, F.KaraListe,Tel,Cep,AnlasmaTarihi,AnlasaBitisTarihi,FG.GrupAdi,F.Devir,F.Eposta";
        ...
        void adindanara()
        {
            ...
            //datediff(dd,SonOdemeTarihi,GETDATE()) as OdemeYapmadigiGun
            //FROM  Firmalar with(nolock)
            //LEFT JOIN(SELECT ADI, KODU FROM  IL_ILCE_MAH with(nolock) WHERE GRUP = '1') AS il ON Firmalar.fkil = il.KODU 
            //LEFT JOIN (SELECT ADI, KODU,ALTGRUP FROM  IL_ILCE_MAH with(nolock) WHERE GRUP = '2') AS ilce ON Firmalar.fkil = ilce.ALTGRUP AND Firmalar.fkilce = ilce.KODU 
            //LEFT JOIN FirmaGruplari with(nolock) ON Firmalar.fkFirmaGruplari = FirmaGruplari.pkFirmaGruplari 
            ...
            left join KasaHareket kh with(nolock) on kh.fkFirma = F.pkFirma
            LEFT JOIN FirmaGruplari FG with(nolock) ON FG.pkFirmaGruplari = F.fkFirmaGruplari
            where F.Firmaadi like '%" + adiara.Text + "%'";

            sql+= "group by F.pkFirma,F.Firmaadi,F.Yetkili, F.KaraListe,Tel,Cep,AnlasmaTarihi,AnlasaBitisTarihi," +
                "FG.GrupAdi,F.Devir,F.Eposta";
...
LEFT JOIN FirmaAltGruplari ON Firmalar.fkFirmaAltGruplari = FirmaAltGruplari.pkFirmaAltGruplari ";
        ...
        private void gridView3_RowCellStyle(object sender, DevExpress.XtraGrid.Views.Grid.RowCellStyleEventArgs e)
        {
            ...
            if (e.Column.FieldName == "kalangun" && int.Parse(dr["kalangun"].ToString())< 0)
              AppearanceHelper.Apply(e.Appearance, appError);
        }
        ...
        private void toolStripMenuItem10_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    //string ACIKLAMA = dt.Rows[i]["ACIKLAMA"].ToString();
                    //string IL = dt.Rows[i]["IL"].ToString();
                    //string ILCE = dt.Rows[i]["ILCE"].ToString();
                    ...
                    //{
                        ...
                        string sql = "INSERT INTO Firmalar (OzelKod,Firmaadi,Adres,Yetkili,FaturaUnvani,fkFirmaGruplari,Devir,Aktif,KayitTarihi,VergiDairesi,VergiNo,Tel,Cep,Fax)" +
                        " values(@OzelKod,@Firmaadi,@Adres,@Yetkili,@FaturaUnvani,@fkFirmaGruplari,@Devir,1,getdate(),@VergiDairesi,@VergiNo,@Tel,@Cep,@Fax) select IDENT_CURRENT('Firmalar')";
        ...
        private void btnGelisEkle_Click(object sender, EventArgs e)
        {
            if (gridView3.FocusedRowHandle < 0) return;
            ...
            DB.ExecuteSQL("insert into FirmaGelisleri (fkFirma,gelis_tarihi) values(" + pkFirma+",getdate())");

            MustetiGelisleri();

        }

        private void MustetiGelisleri()
        {
            if (gridView3.FocusedRowHandle < 0) return;
            ...
            gcSonAlisVeris.DataSource = DB.GetData("select * from FirmaGelisleri with(nolock) where fkfirma=" + pkFirma);
        }

        private void gelişToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (gridView1.FocusedRowHandle < 0) return;

            DataRow dr = gridView1.GetDataRow(gridView1.FocusedRowHandle);

            string musteri_gelis_id = dr["musteri_gelis_id"].ToString();

            DB.ExecuteSQL("delete from FirmaGelisleri where musteri_gelis_id=" + musteri_gelis_id);
