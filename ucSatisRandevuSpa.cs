...
namespace GPTS
{
    ...
    public partial class ucSatisRandevuSpa : DevExpress.XtraEditors.XtraUserControl
    {
        ...
        private void ucAnaEkran_Load(object sender, EventArgs e)
        {
            ...
            #region grid tasarımlar
            //satış detay
            string Dosya = DB.exeDizini + "\\SatisFaturaSpaGrid.xml";
            ...
            //müşteri randevuları
            string Dosya1 = DB.exeDizini + "\\SiparisRandevuListeGrid.xml";
            if (File.Exists(Dosya1))
            {
                gridView4.RestoreLayoutFromXml(Dosya1);
            ...
            //deilktar.DateTime = DateTime.Today;
            //desontar.DateTime = DateTime.Today.AddDays(1);
            cbTarihAraligi.SelectedIndex = 0;
            //MusteriSiparisleri();
            
            PersonelGridComboboxDoldur();

            yesilyak();
            
        }

        ses_islemleri _sesislemleri;
        ...
        void RandevuDurumlar()
        {
            repositoryItemLookUpEdit5.DataSource = DB.GetData(@"select * from HatirlatmaDurum with(nolock) where aktif=1");
            //repositoryItemLookUpEdit5.EditValue = Degerler.fkDonemler;
        }
        ...
        {
            repositoryItemLuePers.DataSource = DB.GetData("select * from Depolar with(nolock)");
            //repositoryItemLookUpEdit3.EditValue = 1;
        }
        void PersonelGridComboboxDoldur()
        {
            //repositoryItemCheckedComboBoxEdit1.DataSource = DB.GetData("select * from Personeller with(nolock)");
            repositoryItemLuePers.DataSource = DB.GetData("select pkPersoneller, adi + ' ' + soyadi as AdiSoyadi from Personeller with(nolock) where AyrilisTarihi is null ");
            repositoryItemLuePers.DisplayMember = "AdiSoyadi";
            repositoryItemLuePers.ValueMember = "pkPersoneller";
            //repositoryItemLookUpEdit3.EditValue = 1;
        }
        ...
        private void simpleButton1_Click(object sender, EventArgs e)
        {
            ...
            string _pkSatisDetay = dr["pkSatisDetay"].ToString();
            int sonuc = 0;
            sonuc = DB.ExecuteSQL("delete from Hatirlatma  where pkHatirlatma=(select fkHatirlatma from SatisDetayRandevu with(nolock) where fkSatisDetay="+ _pkSatisDetay + ")");
            if (sonuc != 1)
            {
                //MessageBox.Show("Hatirlatma Silinmedi " + sonuc);
                //return;
            }
            sonuc = DB.ExecuteSQL("DELETE FROM SatisDetayRandevu WHERE fkSatisDetay=" + _pkSatisDetay);
            if (sonuc != 1)
            {
                MessageBox.Show("Satis Detay Randevu silinmedi " + sonuc);
                return;
            }

            sonuc = DB.ExecuteSQL("DELETE FROM SatisDetay WHERE pkSatisDetay=" + _pkSatisDetay);
            if (sonuc != 1)
            {
                MessageBox.Show("Satış Detay silindi " + sonuc);
                return;
            }
        ...
        void KaydetYazdir(bool yazdirilsinmi)
        {
            ...
            sql = "UPDATE Satislar Set GuncellemeTarihi='" +
                 deGuncellemeTarihi.DateTime.ToString("yyyy-MM-dd HH:mm") + "',AlinanPara=0";
        ...
        private void simpleButton37_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < gridView1.DataRowCount; i++)
            {
                ...
                if (dr["yapan_personeller"].ToString() == "")
                    say++;
            }
            if(say>0)
            {
                DialogResult secim;
                secim = DevExpress.XtraEditors.XtraMessageBox.Show("İşlemi Yapan Personel Seçilmedi. Devam Etmek İstiyormusunuz ?", Degerler.mesajbaslik, MessageBoxButtons.YesNo, MessageBoxIcon.Question, System.Windows.Forms.MessageBoxDefaultButton.Button2);
        ...
        private string musteriara()
        {
            ...
            if (txtpkSatislar.Text != "0" && txtpkSatislar.Text != "")
            {
                ...
                DB.ExecuteSQL("update Hatirlatma set fkFirma=" + fkFirma +
" where pkHatirlatma in (select fkHatirlatma from SatisDetayRandevu where fkSatisDetay in ("+
" select pkSatisDetay from SatisDetay with(nolock) where fkSatislar = "+txtpkSatislar.Text+"))");
            }
            DataTable dt = null;
            try
            {
        ...
        private void gridView1_RowCellStyle(object sender, RowCellStyleEventArgs e)
        {
            ...
            else if (e.Column.FieldName == "RandevuBaslangic")
            {
        ...
        private void sonrakiSatisToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (gridView4.FocusedRowHandle == gridView4.DataRowCount-1)
                gridView4.FocusedRowHandle = 0;
            else
                gridView4.FocusedRowHandle = gridView4.FocusedRowHandle + 1;
            ...
            gridView4_RowClick(sender, null);
        }
...
";
        ...
        private void gridView1_MouseDown(object sender, MouseEventArgs e)
        {
            ...
            if (ghi.InRowCell)
            {
                ...
                else if (ghi.Column.FieldName == "RandevuBaslangic")
                {
                    //gcAciklamaDetay
                    DataRow dr = gridView1.GetDataRow(ghi.RowHandle);
                    ...
                    frmRandevuSec RandevuSec = new frmRandevuSec(pkSatisDetay, Satis1Firma.Tag.ToString(), txtpkSatislar.Text);
                    RandevuSec.ShowDialog();

                    GuncellemeTarihiRandevuZamaniYap();
...
left join SatisDurumu sdu with(nolock) on sdu.pkSatisDurumu=s.fkSatisDurumu
        ...
        private void teklifDizaynToolStripMenuItem1_Click(object sender, EventArgs e)
        {
            ...
            if (YaziciAdi == "")
                ...
                FisYazdirSiparis(3, txtpkSatislar.Text, "Siparis",YaziciAdi);
        }
        ...
        private void vSatisOdeme(object sender, EventArgs e)
        {
            ...
            Satis1Baslik.ForeColor = System.Drawing.Color.Black;
                ...
                case (int)Degerler.OdemeSekli.AcikHesap:
                    {
                        ...
                        cbOdemeSekli.ForeColor = System.Drawing.Color.Silver;

                        //if (AcikSatisindex == 1)
                        //{
                        //    Satis1Baslik.BackColor = System.Drawing.Color.Red;
                        //    Satis1Baslik.ForeColor = System.Drawing.Color.Silver;
                        //}
        ...
        void SatisDetayGetir_Yeni2(string pkSatislar)
        {
            ...
            gcSatisDetay.DataSource = DB.GetData("exec sp_SatisDetayRandevu " + pkSatislar + ",0");
        ...
        void MusteriSiparisleri()
        {
            ...
            string sql = @"select s.pkSatislar,min(h.Tarih) as Tarih,s.Aciklama,f.OzelKod,
            f.Firmaadi,s.fkFirma,sum(sd.Adet*(sd.SatisFiyati-sd.iskontotutar)) as Tutar
            from Satislar s with(nolock)
            ...
            where ((h.Tarih is null)or(h.Tarih>'@ilktarih' and h.Tarih<'@sontarih'))";
            if(cbSiparis.Checked)
                sql += " and s.Siparis=1";
            else
                sql += " and s.Siparis=0";

            sql = sql.Replace("@ilktarih", deilktar.DateTime.ToString("yyyy-MM-dd HH:mm"));
            sql = sql.Replace("@sontarih", desontar.DateTime.ToString("yyyy-MM-dd HH:mm"));

            sql = sql + "group by  s.pkSatislar,s.Aciklama,f.OzelKod,f.Firmaadi,s.fkFirma";
        ...
        private void simpleButton11_Click_1(object sender, EventArgs e)
        {
           ...
           " where pkSatislar=" + txtpkSatislar.Text);
            ...
            DataTable dtYazicilar =
            DB.GetData("SELECT  YaziciAdi,Dosya,YazdirmaAdedi FROM SatisFisiSecimi with(nolock) where Sec=1 and fkSatisDurumu=10"); //+ lueSatisTipi.EditValue.ToString());
            ...
            if (YaziciAdi == "")
                ...
                FisYazdirSiparis(1, txtpkSatislar.Text, "Siparis",YaziciAdi);
        }
        ...
        {
            string Dosya = DB.exeDizini + "\\SiparisRandevuListeGrid.xml";
            gridView4.SaveLayoutToXml(Dosya);
        }
        ...
        {
            string fkSatisDetay = "0";
            if (gridView1.FocusedRowHandle >= 0)
            {
                DataRow dr = gridView1.GetDataRow(gridView1.FocusedRowHandle);
                if (dr == null) return;
                fkSatisDetay= dr["pkSatisDetay"].ToString();
            }
            
            frmRandevuSec RandevuSec = new frmRandevuSec(fkSatisDetay, Satis1Firma.Tag.ToString(), txtpkSatislar.Text);
            RandevuSec.ShowDialog();
        ...
        void GuncellemeTarihiRandevuZamaniYap()
        {
          
            DataTable dtMinTarih =
            DB.GetData(@"select Tarih from Hatirlatma
where pkHatirlatma in (select min(sdr.fkHatirlatma) from SatisDetay sd with(nolock)
inner join SatisDetayRandevu sdr with(nolock)  on sdr.fkSatisDetay = sd.pkSatisDetay
where sd.fkSatislar ="+ txtpkSatislar.Text+ ")");
            if (dtMinTarih.Rows.Count > 0 && dtMinTarih.Rows[0]["Tarih"].ToString() != "")
            {
                string sql = @"update Satislar set GuncellemeTarihi = rt.Tarih from(
                select Tarih from Hatirlatma
                where Tarih is not null and  pkHatirlatma in (select TOP 1 sdr.fkHatirlatma from SatisDetay sd with(nolock)
                inner join SatisDetayRandevu sdr with(nolock)  on sdr.fkSatisDetay = sd.pkSatisDetay
                where sd.fkSatislar = @pkSatislar)) rt
                where Satislar.pkSatislar=@pkSatislar";
                sql = sql.Replace("@pkSatislar", txtpkSatislar.Text);
        ...
        private void repositoryItemLookUpEdit5_EditValueChanged(object sender, EventArgs e)
        {
            ...
            DB.ExecuteSQL("update Hatirlatma set fkDurumu=" + value + " where pkHatirlatma=" + dr["fkHatirlatma"].ToString());

            btnRandevular_Click(sender,e);//yenile
        }
               ...
               int g2, int m2, int y2, int h2, int min2, int sec2, Boolean p2)
        {
            ...
            deilktar.DateTime = d1.AddSeconds(sec1);
            deilktar.ToolTip = deilktar.DateTime.ToString();
            ...
            desontar.DateTime = d2.AddSeconds(sec2);
            desontar.ToolTip = desontar.DateTime.ToString();
        }

        private void cbTarihAraligi_SelectedIndexChanged(object sender, EventArgs e)
        {
            deilktar.Properties.EditMask = "D";
            desontar.Properties.EditMask = "D";
            deilktar.Properties.DisplayFormat.FormatString = "D";
            deilktar.Properties.EditFormat.FormatString = "D";
            desontar.Properties.DisplayFormat.FormatString = "D";
            desontar.Properties.EditFormat.FormatString = "D";
            pTarihAraligi.Visible = false;

            DateTime haftabasi = getStartOfWeek(false);
            if (cbTarihAraligi.SelectedIndex == 0)// Bu gün
            {
                deilktar.Properties.DisplayFormat.FormatString = "f";
                desontar.Properties.EditFormat.FormatString = "f";
                deilktar.Properties.EditFormat.FormatString = "f";
                desontar.Properties.DisplayFormat.FormatString = "f";
                deilktar.Properties.EditMask = "f";
                desontar.Properties.EditMask = "f";
            ...
            {
                desontar.Enabled = true;
                deilktar.Enabled = true;
            }
            else if (cbTarihAraligi.SelectedIndex == 7)
                pTarihAraligi.Visible = true;
        ...
        private void repositoryItemLuePers_EditValueChanged(object sender, EventArgs e)
        {
            if (gridView1.FocusedRowHandle < 0) return;
            ...
            DB.ExecuteSQL("update SatisDetayRandevu set yapan_personeller=" + value + " where fkSatisDetay=" + dr["pkSatisDetay"].ToString());
        ...
        private void btnTamam_Click(object sender, EventArgs e)
        {
            ...
            //if (aratoplam.EditValue == null)
            //sql = sql + "ToplamTutar="+aratoplam.Value.ToString().Replace(",",".");
            //sql = sql + " where pkSatislar=" + txtpkSatislar.Text;

            //DB.ExecuteSQL_Sonuc_Sifir(sql);

            btnYeniSiparis_Click(sender, e);

            MusteriSiparisleri();
        }

        private void cbSiparis_CheckedChanged(object sender, EventArgs e)
        {
            MusteriSiparisleri();
        }
    }
