...
namespace GPTS
{
    ...
    public partial class frmSatisYeni : DevExpress.XtraEditors.XtraForm
    {
        ...
        string _satisid = "0";
        public frmSatisYeni(string firmaid,string satisid)
        {
            InitializeComponent();
        ...
        private void frmSatisYeni_Load(object sender, EventArgs e)
        {
            #region Şirketler
        ...
        void SatisDetayEkle(string barkod)
        {
            ...
            //ürün eklemeden önce yukarda kontrol ediliyor dahaonceeklendimi
        ...
        private void timer1_Tick(object sender, EventArgs e)
        {
            ...
            DataTable dt = DB.GetData("select pkSatislar,fkFirma from Satislar with(nolock) where pkSatislar=" + _satisid);
        ...
        void FisSatisAlisFiyatlarininiDegis(string pkStokKarti)
        {
            ...
            if (sec == "1")
            {
                for (int s = 0; s < gridView1.DataRowCount; s++)
                {
                    ...
                    where SatisDetay.fkStokKarti=StokKarti.pkStokKarti and pkSatisDetay=" + pkSatisDetay);
                }
...
left join SatisDurumu sdu with(nolock) on sdu.pkSatisDurumu=s.fkSatisDurumu
        ...
        void toplu_kdv_iskonto_degis(string ki)
        {
            ...
            //kdv oranı
            if (ki == "k")
                sonuc = DB.ExecuteSQL_Sonuc_Sifir("update SatisDetay set " +
                    "SatisFiyatiKdvHaric = Satisfiyati - ((SatisFiyati * KdvOrani) / (100 + kdvorani))," +
                    "KdvOrani=" + girilen +
                    " where fkSatislar=" + txtpkSatislar.Text);
            //update SatisDetay set SatisFiyatiKdvHaric = Satisfiyati - ((SatisFiyati * KdvOrani) / (100 + kdvorani))
            if (sonuc != 0)
            {
        ...
        private void simpleButton11_Click(object sender, EventArgs e)
        {
            ...
            //MessageBox.Show(pkFirma);
            DataTable dt = DB.GetData("select MAX(pkSatislar) as pkSatislar from Satislar with(nolock) where Siparis=1");
            if (dt.Rows.Count == 0)
            {
                formislemleri.Mesajform("Daha Önce Satış Yapılmamış", "K", 200);
                return;
        ...
        private void btnKdvHaricDahilYap_Click(object sender, EventArgs e)
        {
            ...
            DataTable dtKdv =
            DB.GetData("select fkSatislar from SatisDetay with(nolock) where Satisfiyati=SatisFiyatiKdvHaric and fkSatislar=" + txtpkSatislar.Text);
            if (dtKdv.Rows.Count == 0)
            {
                dahaoncebasilmis = true;
            }
            string mesaj = "Satış Fiyatları Kdv Oranı Kadar Eksilecek Eminmisiniz?";
            if (dahaoncebasilmis)
            {
            ...
            for (int i = 0; i < dtSatisDetay.Rows.Count; i++)
            {
                ...
                if (dahaoncebasilmis)
                {
                    ...
                    iskontoyuzdetutar = 0,Satisfiyati = sk.SatisFiyati,
        ...
        private void gbBaslik_Paint(object sender, PaintEventArgs e)
        {
