...
namespace GPTS
{
    ...
    public partial class frmSatis : DevExpress.XtraEditors.XtraForm
    {
        ...
        int _Firmaid=1, _Satislarid=0;
        string _masaadi = "", _masa_id;

        public frmSatis(int Firmaid,int Satislarid,string masaadi, string masa_id)
        {
            InitializeComponent();
            DB.PkFirma = Firmaid;
            _Firmaid = Firmaid;
            if (_Firmaid == 0) _Firmaid = 1;
            _Satislarid = Satislarid;
            Satis1Toplam.Tag = _Satislarid;
            txtpkSatislar.Text = _Satislarid.ToString();
            _masaadi = masaadi;
            _masa_id = masa_id;
        ...
        private void ucAnaEkran_Load(object sender, EventArgs e)
        {
            ...
            Satis1Toplam.Tag = _Satislarid;
            Satis1ToplamGetir(1);
            musteriataId(_Firmaid.ToString());
        ...
        void Masalar()
        {
            ...
            lueMasalar.EditValue = int.Parse(_masa_id);
        }
...
WHERE ya.fkKullanicilar=" + DB.fkKullanicilar;
        ...
        void GeciciMusteriDefault()
        {
            ...
            DataTable dtMusteriler = DB.GetData("select pkFirma,Firmaadi,OzelKod from Firmalar with(nolock) where pkFirma=" + _Firmaid.ToString());//GeciciMusteri=1");
        ...
        {
            DataTable dtMusteriler = DB.GetData("select pkFirma,Firmaadi,OzelKod from Firmalar with(nolock) where pkFirma="+_Firmaid);
            if (dtMusteriler.Rows.Count == 0)
        ...
        void KaydetYazdir(bool yazdirilsinmi)
        {
            ...
            //test
            //string fkFirma2 =  DB.GetData("select fkFirma from Satislar with(nolock) where pkSatislar=" + 
            //    pkSatisBarkod.Text).Rows[0][0].ToString();
            //if (fkFirma2 != fkFirma)
            //{
            //    MessageBox.Show("Müşteriler Farklı Lütfen Kontrol Ediniz");
            //    return;
            //}
            ...
            sql = sql + ",fkDonemler='" + lueDonemler.EditValue.ToString() + "'";
            ...
            #region Satış Ödeme Kayıt ve yazdır
        ...
        private void simpleButton37_Click(object sender, EventArgs e)
        {
            ...
            KaydetYazdir(false);
            //this.Tag = "Kaydet";
            //formislemleri.Mesajform("Sipariş Kaydedildi", "K", 100);
            this.Hide();
            Close();
            this.Hide();
        ...
        private string musteriara()
        {
            ...
            frmMusteriAraDokunmatik MusteriAra = new frmMusteriAraDokunmatik();
        ...
        private string musteriataOzelKod(string fkFirma)
        {
            string pkFirma = "1", ozelkod = "0", firmadi = "";// MusteriAra.fkFirma.AccessibleDescription;            
            decimal Devir = 0;
            DataTable dt = DB.GetData("select pkFirma,Firmaadi,OzelKod,isnull(Devir,0) as Devir from Firmalar with(nolock) where OzelKod=" + _Firmaid);
            if (dt.Rows.Count == 0)
            {
        ...
        private string musteriataId(string fkFirma)
        {
            string pkFirma = "1", ozelkod = "0", firmadi = "";// MusteriAra.fkFirma.AccessibleDescription;            
            decimal Devir = 0;
            DataTable dt = DB.GetData("select pkFirma,Firmaadi,OzelKod,isnull(Devir,0) as Devir from Firmalar with(nolock) where pkFirma=" + _Firmaid);
            if (dt.Rows.Count == 0)
            {
                DevExpress.XtraEditors.XtraMessageBox.Show("Müşteri Bulunamadı.", Degerler.mesajbaslik, MessageBoxButtons.OK, MessageBoxIcon.Warning);
                Satis1Baslik.Text = "";
        ...
        private void gridView1_RowCellStyle(object sender, RowCellStyleEventArgs e)
        {
            ...
            else if (e.Column.FieldName == "SatisFiyatiiskontolu")
            {
                decimal isk = 0;
                decimal.TryParse(dr["iskontotutar"].ToString(), out isk);
                if (isk > 0)
        ...
        {
            DB.ExecuteSQL("update Satislar set ToplamTutar=" + aratoplam.Text.Replace(",", ".") +
                " where pkSatislar=" + txtpkSatislar.Text);
        ...
        private void lueSatisTipi_EditValueChanged(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                //if (deFaturaTarihi.EditValue == null && !ilkyukleme)
                //deFaturaTarihi.DateTime = DateTime.Today;
                cbGecmisFisler.Text = "SON SATIŞLAR";
                //gbBaslik.Text = gbBaslik.Tag.ToString()+ " Siparişleri ";
                //gbBaslik.Text = _Satislarid +  ".Sipariş ["+ _masaadi+"]";
                gbBaslik.Text = "[" + _masaadi + "] Sipariş";
            }
        ...
        void SatisDetayEkle(string barkod)
        {
            ...
            if (cbOncekiFiyatHatirla.Checked)
            {
                ...
                if (pkFirma != "1")
                {
                    string sql = @"select  top 1 sd.SatisFiyati-isnull(iskontotutar,0) as EskiSatisFiyati from Satislar s with(nolock)
                    inner join SatisDetay sd on s.pkSatislar=sd.fkSatislar
                    inner join StokKarti sk with(nolock) on sk.pkStokKarti = sd.fkStokKarti
                    where sk.satis_iskonto=0 and s.Siparis=1 and s.fkFirma=@fkFirma and  sd.fkStokKarti=@fkStokKarti
                    ...
                    if (dtSonFiyat.Rows.Count > 0)
                    {
                        ...
                        int sd = DB.ExecuteSQL("update SatisDetay set iskontotutar=SatisFiyati-" + EskiSatisFiyati.ToString().Replace(",", ".") +
        ...
        void YeniSatis()
        {
            ...
            list.Add(new SqlParameter("@fkMasa", _masa_id));
        ...
        private void Satis3Baslik_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                if (Satis3Baslik.Text == "")
                    ...
                    musteriataOzelKod(Satis3Baslik.Text);
            }
        ...
        private void Satis2Baslik_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                if (Satis2Baslik.Text == "")
                    ...
                    musteriataOzelKod(Satis2Baslik.Text);
            }
            else if (e.KeyCode == Keys.Escape)
            {
                //musteriata("1");
        ...
        private void Satis1Baslik_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                if (Satis1Baslik.Text == "")
                    ...
                    musteriataOzelKod(Satis1Baslik.Text);
            }
            else if (e.KeyCode == Keys.Escape)
            {
                //musteriata("1");
        ...
        void YazdirTeklif(bool Disigner)
        {
            try
            {
                ...
                string sql = @"SELECT  s.pkSatislar, s.Tarih, s.fkFirma, s.Aciklama, 
                      ...
                      ,dbo.fnc_ParayiYaziyaCevir(sum(sd.Adet*(sd.SatisFiyati-sd.iskontotutar)),1) as rakamoku,s.FaturaTarihi,s.TeslimTarihi,B.BankaAdi
FROM  Satislar s with(nolock)
LEFT JOIN Bankalar B with(nolock)  ON B.PkBankalar=s.fkBankalar
INNER JOIN SatisDetay sd with(nolock) on sd.fkSatislar=s.pkSatislar
...
s.FaturaTarihi,s.TeslimTarihi,B.BankaAdi";

                //sql = sql.Replace("@pkSatislar", fisid);
                sql = "exec sp_Satislar " + fisid;
...
";
        ...
        private void btnGecmisFisler_Click(object sender, EventArgs e)
        {
            ...
            if (lueSatisTipi.EditValue.ToString() == ((int)Degerler.SatisDurumlari.Teklif).ToString())
            {
                ...
                s.OdemeSekli,k.KullaniciAdi,Sd.Durumu,s.Yazdir as Yazdırıldı from Satislar s with(nolock)
            ...
            {
                sql = @"select s.fkFirma,pkSatislar as FisNo,f.Firmaadi as MusteriAdi,cast(round(ToplamTutar, 3, 1) as decimal(18,2)) as ToplamTutar,s.GuncellemeTarihi as Tarih,
                s.OdemeSekli,k.KullaniciAdi,Sd.Durumu,s.TeslimTarihi,s.Yazdir as Yazdırıldı from Satislar s with(nolock)
                LEFT JOIN Firmalar f with(nolock) ON s.fkFirma = f.PkFirma
                LEFT JOIN Kullanicilar k with(nolock) ON s.fkKullanici=k.pkKullanicilar
                LEFT JOIN SatisDurumu sd with(nolock) ON sd.pkSatisDurumu=s.fkSatisDurumu
            ...
            {
                sql = @"select Top 25 s.fkFirma,pkSatislar as FisNo,f.Firmaadi as MusteriAdi,cast(round(ToplamTutar, 3, 1) as decimal(18,2)) as ToplamTutar,s.GuncellemeTarihi as Tarih,
s.OdemeSekli,k.KullaniciAdi,Sd.Durumu,s.TeslimTarihi,s.Yazdir as Yazdırıldı from Satislar s with(nolock)
LEFT JOIN Firmalar f with(nolock) ON s.fkFirma = f.PkFirma
LEFT JOIN Kullanicilar k with(nolock) ON s.fkKullanici=k.pkKullanicilar
LEFT JOIN SatisDurumu sd with(nolock) ON sd.pkSatisDurumu=s.fkSatisDurumu
where s.Siparis=1 and s.fkSatisDurumu>1 and s.GuncellemeTarihi>getdate()-30";
        ...
        private void gcSatisDetay_MouseDoubleClick(object sender, MouseEventArgs e)
        {
            ...
            if (ghi.Column.FieldName == "Stokadi")
            {
                ...
                frmSatisDetayDuzelt SatisUrunBazindaDetay = new frmSatisDetayDuzelt(int.Parse(fkSatisDetay));
...
left join SatisDurumu sdu with(nolock) on sdu.pkSatisDurumu=s.fkSatisDurumu
        ...
        private void btnyazdir_Click(object sender, EventArgs e)
        {
            ...
            else if (int.Parse(lueSatisTipi.EditValue.ToString()) == (int)Degerler.SatisDurumlari.Tevkifat)
            {
                if (deFaturaTarihi.EditValue == null)
                    deFaturaTarihi.DateTime = DateTime.Now;
        ...
        private void TeklifYazdir(Int16 dizayner)
        {
            string YaziciAdi = "", YaziciDosyasi = "";
           bool onizle = false;
            DataTable dtYazicilar =
            DB.GetData("SELECT  YaziciAdi,Dosya,YazdirmaAdedi,onizle FROM SatisFisiSecimi with(nolock) where Sec=1 and fkSatisDurumu=" + lueSatisTipi.EditValue.ToString());
            ...
            if (dtYazicilar.Rows.Count == 1)
            {
                ...
                short.TryParse(dtYazicilar.Rows[0]["YazdirmaAdedi"].ToString(), out yazdirmaadedi);
                if(dtYazicilar.Rows[0]["Dosya"].ToString()=="1")
                    onizle = true;
            }
            else if (dtYazicilar.Rows.Count > 1)
            {
                ...
                frmYaziciAyarlari YaziciAyarlari = new frmYaziciAyarlari(1,(int)Degerler.SatisDurumlari.Teklif);
                YaziciAyarlari.ShowDialog();
                YaziciAyarlari.Tag = 0;
        ...
        private void btnFisKopyala_Click(object sender, EventArgs e)
        {
            ...
            sql = "INSERT INTO Satislar (Tarih,fkFirma,Siparis,GelisNo,fkKullanici,fkSatisDurumu,Aciklama,AlinanPara,ToplamTutar,Yazdir,iskontoFaturaTutar,AcikHesap,AcikHesapOdenen,OdemeSekli,SonislemTarihi,BilgisayarAdi,fkSatisFiyatlariBaslik)" +
                " VALUES(getdate(),@fkFirma,@Siparis,1,@fkKullanici,@fkSatisDurumu,@Aciklama,@AlinanPara,@ToplamTutar,@Yazdir,@iskontoFaturaTutar,0,0,@OdemeSekli,getdate(),@BilgisayarAdi,@fkSatisFiyatlariBaslik) SELECT IDENT_CURRENT('Satislar')";
            fisno = DB.ExecuteScalarSQL(sql, list);
        ...
        {
            DB.ExecuteSQL("UPDATE Satislar set FaturaTarihi='" + deFaturaTarihi.DateTime.ToString("yyyy-MM-dd") + "' where pkSatislar=" + txtpkSatislar.Text);
            //deFaturaTarihi.
        }
        ...
        {
            frmMasaDegis degis = new frmMasaDegis();
            degis.ShowDialog();
        }
        ...
        {
            ceAdet.Text = ceAdet.Text+"1";
        }

        private void simpleButton24_Click(object sender, EventArgs e)
        {
            ceAdet.Text = ceAdet.Text + "2";
        }
        ...
        private void lueMasalar_EditValueChanged(object sender, EventArgs e)
        {
            if (lueMasalar.Tag.ToString() == "1")
            {
                ...
                DB.ExecuteSQL("update Masalar set fkSatislar=0 where pkMasalar="+ _masa_id);
